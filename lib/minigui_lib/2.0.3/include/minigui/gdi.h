...
/*
 * $Id: gdi.h 6913 2006-06-26 04:00:44Z xwyan $
 *
...
#define SetBkColor(hdc, color)  \
                (gal_pixel) SetDCAttr (hdc, DC_ATTR_BK_COLOR, (DWORD) color)
...
#define SetBkMode(hdc, mode)    \
                (int) SetDCAttr (hdc, DC_ATTR_BK_MODE, (DWORD) mode)
...
#define SetPenColor(hdc, color) \
                (gal_pixel) SetDCAttr (hdc, DC_ATTR_PEN_COLOR, (DWORD) color)
...
#define SetBrushColor(hdc, color)   \
                (gal_pixel) SetDCAttr (hdc, DC_ATTR_BRUSH_COLOR, (DWORD) color)
...
#define SetTextColor(hdc, color)    \
                (gal_pixel) SetDCAttr (hdc, DC_ATTR_TEXT_COLOR, (DWORD) color)
...
#define SetTabStop(hdc, value)  \
                (int) SetDCAttr (hdc, DC_ATTR_TAB_STOP, (DWORD) value)
 ...
 */
MG_EXPORT BOOL GUIAPI PolygonGenerator (void* context, 
                const POINT* pts, int vertices, CB_POLYGON cb);
...
#define SetPenType(hdc, type)           \
                (PenType) SetDCAttr (hdc, DC_ATTR_PEN_TYPE, (DWORD) type)
...
#define SetPenCapStyle(hdc, style)          \
            (PTCapStyle) SetDCAttr (hdc, DC_ATTR_PEN_CAP_STYLE, (DWORD) style)
...
#define SetPenJoinStyle(hdc, style)         \
            (PTJoinStyle) SetDCAttr (hdc, DC_ATTR_PEN_JOIN_STYLE, (DWORD) style)
...
#define GetPenWidth(hdc)                    \
                (unsigned int) GetDCAttr (hdc, DC_ATTR_PEN_WIDTH)
...
#define SetPenWidth(hdc, width)             \
                (unsigned int) SetDCAttr (hdc, DC_ATTR_PEN_WIDTH, (DWORD) width)
...
#define SetBrushType(hdc, type)         \
                (BrushType) SetDCAttr (hdc, DC_ATTR_BRUSH_TYPE, (DWORD) type)
...
#define SetMapMode(hdc, mapmode)        \
                (int)SetDCAttr (hdc, DC_ATTR_MAP_MODE, (DWORD)mapmode)
 ...
 *
 * This function retrives mapping paramters of the specified device context 
 * \a hdc when the mapping mode is not \a MM_TEXT.
 *
 * \param hdc The device context.
 * \param which Which parameter you want to retrive, can be one of 
 *        the following values:
 *
 *      - DC_LCS_VORG\n
 ...
 *
 * This function retrives information of multi-byte words in the string 
 ...
 *
 * This function retrives the information of the first multi-byte character 
...
/**
 * \fn int GUIAPI MB2WC (PLOGFONT log_font, \
 ...
 * \sa WC2MB, mbtowc
 */
MG_EXPORT int GUIAPI MB2WC (PLOGFONT log_font, 
                wchar_t* dest, const unsigned char* mstr, int n);

/**
 * \fn int GUIAPI WC2MB (PLOGFONT log_font, unsigned char *s, wchar_t wc)
 *
 ...
 * \sa WC2MB, mbtowc
 */
MG_EXPORT int GUIAPI WC2MB (PLOGFONT log_font, unsigned char *s, wchar_t wc);

/**
 * \fn int GUIAPI MBS2WCS (PLOGFONT log_font, wchar_t* dest, \
 ...
 * \sa WCS2MBS, mbstowcs
 */
MG_EXPORT int GUIAPI MBS2WCS (PLOGFONT log_font, wchar_t* dest, 
                const unsigned char* mstr, int mstr_len, int n);

/**
 * \fn int GUIAPI WCS2MBS (PLOGFONT log_font, unsigned char* dest, \
 ...
 * \sa MBS2WCS, wcstombs
 */
MG_EXPORT int GUIAPI WCS2MBS (PLOGFONT log_font, unsigned char* dest, 
                const wchar_t *wcs, int wcs_len, int n);
 ...
 *
 * This function retrives the height of the current logical font in 
 * the DC \a hdc.
 *
 * \param hdc The device context.
 ...
 * \brief Retrives the maximal character width of the current logical font 
 *        in a DC.
 *
 * This function retrives the maximal character width of the current 
...
#define SetTextCharacterExtra(hdc, extra)       \
                (int)SetDCAttr (hdc, DC_ATTR_CHAR_EXTRA, (DWORD) extra)
...
#define SetTextAboveLineExtra(hdc, extra)       \
                (int)SetDCAttr (hdc, DC_ATTR_ALINE_EXTRA, (DWORD) extra)
...
#define SetTextBellowLineExtra(hdc, extra)      \
                (int)SetDCAttr (hdc, DC_ATTR_BLINE_EXTRA, (DWORD) extra)
 ...
 *  - DT_WORDBREAK\n
 *    Break words. Lines are automatically broken beteween words if a word 
 ...
 *  - DT_CHARBREAK\n
 *    Break characters. Lines are automatically broken beteween characters 
 ...
 *  - DT_SINGLELINE\n
 *    Display text ont the single line only. Carriage returns and linefeeds 
 *    do not break the line.
 *  - DT_EXPANDTABS\n
 *    Expands tab characters. The default number of character per tab is eight.
 *  - DT_TABSTOP\n
 *    Set tab stops. Bits 15-8 (hight-order byte of the low-order word) of 
